...
data_config = dict(
    type="ScanNetDataset",
    split="val",
    data_root="/home/fuxiaowen/DLproject/Pointcept-main/PROCESSED_DIR/PROCESSED_SCANNET_DIR",
    transform=[
...
color_map = np.array([
    [255, 0, 0],    # 类别 0 对应的颜色 (红色)
    [0, 255, 0],    # 类别 1 对应的颜色 (绿色)
    [0, 0, 255],    # 类别 2 对应的颜色 (蓝色)
    [255, 255, 0],  # 类别 3 对应的颜色 (黄色)
    [255, 0, 255],  # 类别 4 对应的颜色 (紫色)
    [0, 255, 255],  # 类别 5 对应的颜色 (青色)
    [128, 128, 128],# 类别 6 对应的颜色 (灰色)
    [128, 0, 0],    
    [0, 128, 0],    
    [0, 0, 128],    
    [128, 128, 0],  
...
class SemSegTester(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            print(f"segment:{segment.shape}")# segment形状与pred刚好相同,num_classes始终是20
            ...
            if os.path.isfile(pred_save_path):
                ...
                coord = torch.zeros((segment.size, 3)).cuda() # 0915 采取与pred类似的方式获取coord

                # print(f"fragment_list: length-{len(fragment_list)}") # 就是batch_size
                for i in range(len(fragment_list)): # fragment_list有可能是不同的序列,也有可能是切分成不同的片段，充分利用信息并减小计算成本
                    fragment_batch_size = 1
                    s_i, e_i = i * fragment_batch_size, min(
                        (i + 1) * fragment_batch_size, len(fragment_list)
                    )
                    input_dict = collate_fn(fragment_list[s_i:e_i]) # 取出其中一个序列得到数据input_dict
                    ...
                    with torch.no_grad():
                        ...
                        for be in input_dict["offset"]:
                            ...
                            coord[idx_part[bs:be], :] = input_dict["coord"][bs:be] # 0915
                            bs = be
                    logger.info(
                ...
                if self.cfg.data.test.type == "ScanNetPPDataset":
                    ...
                    pred = pred.max(1)[1].data.cpu().numpy() # pred.max(1)代表在第1维取最大值，返回两个值，分别是最大值与所在索引即类别编号
                    print(f"coord:{coord.shape}; pred:{pred.shape}")
                    color = color_map[pred]
                    color = torch.from_numpy(color).float().cuda() / 255.0
                    save_point_cloud(
                        coord = coord,
                        color = color,
                        file_path=f"../../../exp/scannet/semseg-pt-v3m1-0-base/pictures/pca_colors_{idx}.ply"
                    )   
                    print(f"Generalized pcd {idx + 1}/{len(self.test_loader)}")
                if "origin_segment" in data_dict.keys():
            ...
            if(idx == len(self.test_loader) - 1):
                sort_acc = sorted(self.acc.items(), key = lambda item:item[1], reverse=True)
                sort_miou = sorted(self.miou.items(), key = lambda item:item[1], reverse=True)
                print(f"acc_most: {sort_acc[0]}")
                print(f"miou_most: {sort_miou[0]}")
        ...
        if comm.is_main_process():
            ...
            iou_class = intersection / (union + 1e-10)
            # print(f"iou_class:{iou_class}; intersection:{intersection}; target:{target}")
