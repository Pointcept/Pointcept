...
class MaskedSceneContrast(nn.Module):
    ...
    def compute_contrastive_loss(self, view1_feat, view1_coord, view1_offset,
                                 view2_feat, view2_coord, view2_offset,
                                 match_index):
        assert view1_offset.shape == view2_offset.shape
        device = view1_feat.device
        loss = torch.tensor(0., device=device)
        pos_sim = torch.tensor(0., device=device)
        neg_sim = torch.tensor(0., device=device)
        large_num = 1e9
        ...
        for batch_id in batch.unique():
            ...
            part = self.compute_partitions(view1_coord[batch_mask], view2_coord[batch_mask])
            for part_id in part.unique():
                part_mask = (part == part_id)
                part_mask.fill_diagonal_(True)
                loss += self.nce_criteria(torch.div(sim, self.nce_t) - large_num*(~part_mask).float(), labels)
    ...
    def forward(self, data_dict):
        ...
        nce_loss, pos_sim, neg_sim = self.compute_contrastive_loss(
            view1_feat, view1_origin_coord, view1_offset,
            view2_feat, view2_origin_coord, view2_offset, match_index
        )
