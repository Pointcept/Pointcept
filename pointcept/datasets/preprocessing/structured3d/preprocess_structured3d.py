...
CLASS_LABELS_25 = ("wall", "floor", "cabinet", "bed", "chair", "sofa", "table", "door", "window", "picture", "desk",
...
class Structured3DReader:
    ...
    def listdir(self, dir_name):
        ...
        file_list = list(np.unique([f.replace(dir_name + os.path.sep, "", 1).split(os.path.sep)[0]
                                    for f in self.filelist() if f.startswith(dir_name + os.path.sep)]))
        if "" in file_list:
            file_list.remove("")
        return file_list
    ...
    def read_depth(self, depth_path):
        depth = cv2.imdecode(np.frombuffer(self.read(depth_path), np.uint8), cv2.IMREAD_UNCHANGED)[..., np.newaxis]
        depth[depth == 0] = 65535
        return depth

    def read_color(self, color_path):
        color = cv2.imdecode(np.frombuffer(self.read(color_path), np.uint8), cv2.IMREAD_UNCHANGED)[..., :3][..., ::-1]
        return color
...
def parse_scene(scene, dataset_root, output_root, ignore_index=-1, grid_size=None,
    ...
    reader = Structured3DReader([os.path.join(dataset_root, f)
                                 for f in os.listdir(dataset_root) if f.endswith(".zip")])
    ...
    for room in rooms:
        ...
        if fuse_prsp:
            ...
            for frame in frames:
                try:
                    cam_r, cam_t, cam_f = reader.read_camera(os.path.join(prsp_path, frame, "camera_pose.txt"))
                    ...
                    coord = (depth.reshape((-1, 1)) * (np.linalg.inv(k) @ pixel.T).T).reshape(height, width, 3)
        ...
        if fuse_pano:
            ...
            try:
                _, cam_t, _ = reader.read_camera(os.path.join(pano_path, "camera_xyz.txt"))
        ...
        if len(coord_list) > 0:
            ...
            if vis:
                ...
                save_point_cloud(coord, color / 255, f"./vis/{scene}_room{room}_color.ply")
                save_point_cloud(coord, (normal + 1) / 2, f"./vis/{scene}_room{room}_normal.ply")
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', default=mp.cpu_count(), type=int, help='Num workers for preprocessing.')
    parser.add_argument('--grid_size', default=None, type=float, help='Grid size for grid sampling.')
    parser.add_argument('--ignore_index', default=-1, type=float, help='Ignore index.')
    parser.add_argument('--fuse_prsp', action="store_true", help='Whether fuse perspective view.')
    parser.add_argument('--fuse_pano', action="store_true", help='Whether fuse panorama view.')
    config = parser.parse_args()

    reader = Structured3DReader([os.path.join(config.dataset_root, f)
                                 for f in os.listdir(config.dataset_root) if f.endswith(".zip")])
    ...
    _ = list(pool.map(parse_scene, scenes_list, repeat(config.dataset_root), repeat(config.output_root),
                      repeat(config.ignore_index), repeat(config.grid_size),
                      repeat(config.fuse_prsp), repeat(config.fuse_pano)))
    pool.shutdown()
